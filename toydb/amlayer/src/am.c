/**
 * @file am.c
 * @brief Implementation of B+ tree split and parent update logic.
 */
...
int AM_SplitLeaf(int fileDesc, char *pageBuf, int *pageNum, int attrLength, int recId, char *value, int status, int index, char *key)
{
	...
	AM_bcopy(tempPage, (char *)tempheader, AM_sl);
	...
	AM_bcopy(tempPageBuf + AM_sl, key, attrLength);

	/*check if the split page is root */
	if ((*pageNum) == AM_RootPageNum)
	{
		/* the page being split is the root*/
		/* Allocate a new page for another leaf as a new root has to be created*/
		errVal = PF_AllocPage(fileDesc, &tempPageNum1, &tempPageBuf1);
		AM_Check;

		AM_LeftPageNum = tempPageNum1; /* this will remain the leftmost page hence*/

		/* copy the old first half(actually the root) into a new page */
		AM_bcopy(pageBuf, tempPageBuf1, PF_PAGE_SIZE);
		/* Initialise the new root page */

		AM_FillRootPage(pageBuf, tempPageNum1, tempPageNum, key, header->attrLength, header->maxKeys);
...
int AM_AddtoParent(int fileDesc, int pageNum, char *value, int attrLength)
{
	...
	int pageNumber;				 /* pageNumber of parent to which key is to be added- got from stack*/
	int offset;					 /* Place in parent where key is to be added - got from stack*/
	int errVal;
	...
	/* initialise header */
	header = &head;
	/* Get the top of stack values for the page number of the parent and offset of the key */
	...
	else
	{
		...
		if (pageNumber == AM_RootPageNum)
		{
			...
			AM_bcopy(tempPage, pageBuf2, PF_PAGE_SIZE);

			/* fill the header of new root page and the attribute value */
			AM_FillRootPage(pageBuf, pageNum2, pageNum1, value, header->attrLength, header->maxKeys);
		...
		else
		{
			...
			/* recursive call to add to the parent of this internal node*/
...
void AM_AddtoIntPage(char *pageBuf, char *value, int pageNum, AM_INTHEADER *header, int offset)
{
	...
	/* shift all the keys greater than the one to be added to the right to make way for new key */
	for (i = header->numKeys; i > offset; i--)
		AM_bcopy(pageBuf + AM_sint + (i - 1) * recSize + AM_si, pageBuf + AM_sint + i * recSize + AM_si, recSize);

	/* copy the attribute value into the appropriate place */
	AM_bcopy(value, pageBuf + AM_sint + offset * recSize + AM_si, header->attrLength);

	/* copy the pagenumber of the child */
	AM_bcopy((char *)&pageNum, pageBuf + AM_sint + (offset + 1) * recSize, AM_si);
...
void AM_FillRootPage(char *pageBuf, int pageNum1, int pageNum2, char *value, short attrLength, short maxKeys)
{
	...
	AM_bcopy((char *)&pageNum1, pageBuf + AM_sint, AM_si);
	AM_bcopy(value, pageBuf + AM_sint + AM_si, attrLength);
	AM_bcopy((char *)&pageNum2, pageBuf + AM_sint + AM_si + attrLength, AM_si);
...
void AM_SplitIntNode(char *pageBuf, char *pbuf1, char *pbuf2, AM_INTHEADER *header, char *value, int pageNum, int offset)
{
	AM_INTHEADER temphead, *tempheader;
	int recSize;
	char tempPage[PF_PAGE_SIZE + AM_MAXATTRLENGTH]; /* temp page for manipulating pageBuf */
	int length1, length2;
	...
	AM_bcopy(pageBuf + AM_sint, tempPage, length1);

	/* copy the key itself */
	AM_bcopy(value, tempPage + length1, header->attrLength);

	/* copy the pagenumber of the child node */
	AM_bcopy((char *)&pageNum, tempPage + length1 + header->attrLength, AM_si);

	length2 = (header->maxKeys - offset) * recSize;

	/* copy the rest of the keys */
	AM_bcopy(pageBuf + AM_sint + length1, tempPage + length1 + header->attrLength + AM_si, length2);
	...
	AM_bcopy(tempPage, pbuf1 + AM_sint, length2);
	tempheader->numKeys = length1;

	/* copy header into pbuf1 */
	AM_bcopy((char *)tempheader, pbuf1, AM_sint);

	/* copy the middle key into value to be passed back to parent */
	AM_bcopy(tempPage + AM_si + length1 * recSize, value, header->attrLength);

	/* copy the second half into pbuf2*/
	AM_bcopy(tempPage + AM_si + length1 * recSize + header->attrLength, pbuf2 + AM_sint, length2);
