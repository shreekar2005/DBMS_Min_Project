...
int main(void)
{
    ...
    for (ch = 'a', recnum = 0; ch <= 'z'; ch = succ(ch), recnum++)
    {
        ...
        padstring(buf, NAMELENGTH);
        xAM_InsertEntry(id0, CHAR_TYPE, NAMELENGTH, buf, IntToRecId(recnum));
    }

    printf("opening index scan on char\n");
    /* * FIX: Use GE_OP (Greater Than or Equal) with a NULL value
     * to scan all records from the beginning of the index.
     */
    sd0 = xAM_OpenIndexScan(id0, CHAR_TYPE, NAMELENGTH, GE_OP, NULL);
    ...
    /* * FIX: Use GE_OP with NULL here as well to scan all integer records.
     */
    sd1 = xAM_OpenIndexScan(id1, INT_TYPE, sizeof(int), GE_OP, NULL);
    printf("retrieving recid's from scan descriptor %d\n", sd1);
    numrec = 0;
