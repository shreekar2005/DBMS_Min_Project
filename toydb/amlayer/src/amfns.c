/**
 * @file amfns.c
 * @brief AM layer functions for creating, destroying, inserting, and deleting entries.
 */
...
int AM_CreateIndex(char *fileName, int indexNo, char attrType, int attrLength)
{
	...
	char indexfName[AM_MAX_FNAME_LENGTH]; /* String to store the indexed files name with extension */
...
int AM_DeleteEntry(int fileDesc, char attrType, int attrLength, char *value, int recId)
{
	...
	int errVal;					 /* holds the return value of functions called within this function */
	int i;						 /* loop index */
	...
	/* find the pagenumber and the index of the key to be deleted if it is there */
	...
	AM_bcopy(currRecPtr, (char *)&nextRec, AM_ss);

	/* search the list for recId */
	while (nextRec != 0)
	{
		AM_bcopy(pageBuf + nextRec, (char *)&tempRec, AM_si);

		/* found the recId to be deleted */
		if (recId == tempRec)
		{
			/* Delete recId */
			AM_bcopy(pageBuf + nextRec + AM_si, currRecPtr, AM_ss);
			header->numinfreeList++;
			oldhead = header->freeListPtr;
			header->freeListPtr = nextRec;
			AM_bcopy((char *)&oldhead, pageBuf + nextRec + AM_si, AM_ss);
			break;
		}
		else
		{
	...
	AM_bcopy(pageBuf + AM_sl + (index - 1) * recSize + attrLength, (char *)&temp, AM_ss);
	if (temp == 0)
	{
		/* list is empty , so delete key from the list */
		for (i = index; i < (header->numKeys); i++)
			AM_bcopy(pageBuf + AM_sl + i * recSize, pageBuf + AM_sl + (i - 1) * recSize, recSize);
		header->numKeys--;
...
int AM_InsertEntry(int fileDesc, char attrType, int attrLength, char *value, int recId)
{
	...
	int inserted;				/* Whether key has been inserted into the leaf or splitting is needed */
	int addtoparent;			/* Whether key has to be added to the parent */
	int errVal;					/* return value of functions within this function */
	char key[AM_MAXATTRLENGTH]; /* holds the attribute to be passed back to the parent */
