/**
 * @file amscan.c
 * @brief Implementation of index scan functions for the AM layer.
 */
...
int AM_OpenIndexScan(int fileDesc, char attrType, int attrLength, int op, char *value)
{
	...
	AM_scanTable[scanDesc].attrLength = attrLength;
	if (value != NULL)
		AM_bcopy(value, AM_scanTable[scanDesc].value, attrLength);
	...
	if (value == NULL)
	{
		...
		AM_Check;
		AM_bcopy(pageBuf + AM_sl + attrLength, (char *)&AM_scanTable[scanDesc].nextRecIdPtr, AM_ss);
	...
	/* value is not in leaf but if inserted will have to be inserted after the last key */
	...
	case EQUAL:
	{
		...
		else
		{
			...
			AM_bcopy(pageBuf + AM_sl + (index - 1) * recSize + attrLength, (char *)&AM_scanTable[scanDesc].nextRecIdPtr, AM_ss);
	...
	case GREATER_THAN:
	{
		if (status == AM_FOUND)
		{
			if ((index + 1) <= (header->numKeys))
			{
				...
				AM_bcopy(pageBuf + AM_sl + (index)*recSize + attrLength, (char *)&AM_scanTable[scanDesc].nextRecIdPtr, AM_ss);
			}
			else
			{
...
int AM_FindNextEntry(int scanDesc)
{
	...
	header = &head;
	errVal = PF_GetThisPage(AM_scanTable[scanDesc].fileDesc, AM_scanTable[scanDesc].nextpageNum, &pageBuf);
	AM_Check;

	AM_bcopy(pageBuf, (char *)header, AM_sl);
	recSize = header->attrLength + AM_ss;

	errVal = PF_UnfixPage(AM_scanTable[scanDesc].fileDesc, AM_scanTable[scanDesc].nextpageNum, FALSE);
	AM_Check;
	...
	while (header->numKeys == 0)
	{
		...
		else
		{
			...
			AM_scanTable[scanDesc].actindex = 1;
			AM_bcopy(pageBuf, (char *)header, AM_sl);
			AM_bcopy(pageBuf + AM_sl + (AM_scanTable[scanDesc].nextIndex - 1) * recSize + header->attrLength, (char *)&AM_scanTable[scanDesc].nextRecIdPtr, AM_ss);
			AM_scanTable[scanDesc].status = FIRST;
		}
	}

	/* if op is < or <= check if you are done - might have overshot while scanning empty pages*/
	if ((AM_scanTable[scanDesc].op == LESS_THAN) || (AM_scanTable[scanDesc].op == LESS_THAN_EQUAL))
	{
		if ((AM_scanTable[scanDesc].lastpageNum <= AM_scanTable[scanDesc].nextpageNum) && (AM_scanTable[scanDesc].lastIndex == 0))
		{
			AM_scanTable[scanDesc].status = OVER;
			return (AME_EOF);
		}
	...
	{
		if ((AM_scanTable[scanDesc].pageNum == AM_scanTable[scanDesc].nextpageNum) && (AM_scanTable[scanDesc].index == AM_scanTable[scanDesc].actindex))
		{
			/*skip this value */
			...
			else
			{
				...
				else
				{
					...
					errVal = PF_GetThisPage(AM_scanTable[scanDesc].fileDesc, header->nextLeafPage, &pageBuf);
					AM_Check;
					AM_bcopy(pageBuf + AM_sl + header->attrLength, (char *)&AM_scanTable[scanDesc].nextRecIdPtr, AM_ss);
					AM_bcopy(pageBuf, (char *)header, AM_sl);
					errVal = PF_UnfixPage(AM_scanTable[scanDesc].fileDesc, header->nextLeafPage, FALSE);
					AM_Check;
				}
	...
	/* if not the first call to findnextentry , check if previous record has been deleted */
	if (AM_scanTable[scanDesc].status != FIRST)
	{
		compareVal = AM_Compare(pageBuf + (AM_scanTable[scanDesc].nextIndex - 1) * recSize + AM_sl, AM_scanTable[scanDesc].attrType, header->attrLength, AM_scanTable[scanDesc].nextvalue);
		if (compareVal != 0)
		{
			/* prev record deleted */
			AM_scanTable[scanDesc].nextIndex--;
	...
	else
	{
		...
		AM_bcopy(pageBuf + AM_sl + (AM_scanTable[scanDesc].nextIndex - 1) * recSize, AM_scanTable[scanDesc].nextvalue, header->attrLength);
	}

	/* copy the recId to be returned */
	AM_bcopy(pageBuf + AM_scanTable[scanDesc].nextRecIdPtr, (char *)&recId, AM_si);

	/* copy the place for next recId */
	AM_bcopy(pageBuf + AM_scanTable[scanDesc].nextRecIdPtr + AM_si, (char *)&AM_scanTable[scanDesc].nextRecIdPtr, AM_ss);
	...
	if ((AM_scanTable[scanDesc].op == LESS_THAN) || (AM_scanTable[scanDesc].op == LESS_THAN_EQUAL))
	{
		if ((AM_scanTable[scanDesc].lastpageNum == AM_scanTable[scanDesc].nextpageNum) && (AM_scanTable[scanDesc].lastIndex == AM_scanTable[scanDesc].actindex))
			AM_scanTable[scanDesc].status = LAST;
		else if ((AM_scanTable[scanDesc].lastpageNum == AM_scanTable[scanDesc].nextpageNum) && (AM_scanTable[scanDesc].lastIndex < AM_scanTable[scanDesc].actindex))
			AM_scanTable[scanDesc].status = OVER;
